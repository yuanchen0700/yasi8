...
- 输入: qa-md-tl-tl/qa-tl-XX.md（听力问答精要，含 ## Qn 与 ### 子标题）
...
QA_MD_DIR = Path(r'D:\a.create\_MY_GIT_lib\GITEE\yasi\yasi-rdtli\qa-md-tl')
...
def build_doc_index() -> list:
    docs = []
    for md_file in sorted(QA_MD_DIR.glob('qa-tl-*.md')):
...
def main():
    ...
    index_html = HTML_TEMPLATE.format(
        ...
        content='<h1>雅思听力 8 讲 · 问答精要</h1>'
                ...
                '左侧选择课程，右侧看本页问题。</p>'
                '<p style="color:#6b7280;">需要完整逐字稿版请查看 <a href="tl-index.html" style="color:#2c5cdc;">听力原版课程页</a>。</p>',
